...
*/
-- 사원명, 부서명, 급여, 연봉 조회
-- 부서 배치가 아직 안된 사원 2명에 대한 정보는 조회가 X
...
/*
    3. 비등가 조인(NON EQUL JOIN)
    매칭시킬 컬럼에 대한 조건 작성시 '='(등호)를 사용하지 않은 조인문
    값의 범위에 포하모디는 행들을 연결하는 방식
    ANSI 구문으로는 JOIN ON 으로만 사용 가능! (USING 사용 불가)
*/
...
SELECT E.EMP_ID 사원사번, E.EMP_NAME 사원명, E.DEPT_CODE 사원부서코드, 
          M.EMP_ID 사수사원, M.EMP_NAME 사수명, M.DEPT_CODE 사수부서코드
FROM EMPLOYEE E, EMPLOYEE M  -- E 사원 /  M 사수 라고 가정
...
/*
    5. 카테시안곱(CARTESIAN PRODUCT) 오라클 / 교차 조인(CROSS JOIN) 표준
...
-- 종합실습 문제
-- 1. 직급이 대리이면서 ASIA 지역에서 근무하는 직원들의 사번, 직원명, 직급명, 부서명, 근무지역,
-- 급여를 조회.
-->> ANSI 구문
...
-->> 오라클 구문
SELECT EMP_NAME, BONUS, SALARY*12, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE E, DEPARTMENT D, LOCATION C
WHERE E.DEPT_CODE = D.DEPT_ID(+)
    AND D.LOCATION_ID = L.LOCAL_CODE(+)
    AND BONUS IS NOT NULL;
...
-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균급여(정수 처리)를 조회
