/*
    ...
    * PL/SQL 구조
    ...
    - 실행부 (EXECUTAVLE SECTION) : BEGIN으로 시작, SQL문 또는 제어문(조건문, 반복문) 
                                                                                등의 로직을 기술하는 부분
    - [ 예외처리부(EXCEPTION SECTION) ] : EXCEPTION으로 시작, 예외 발생시 해결하기 위한 
                                                     구문을 미리 기술해 둘 수 있는 부분
*/
-- 출력 기능 활성화
SET SERVEROUTPUT ON;
...
-- 각 변수의 자료형은 EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE, SALARY
-- 컬럼과 DEPARTMENT 테이블의 DEPT_TITLE 컬럼의 자료형을 참조
...
/*
    1-3) IF ~ ELSEIF ~ ELSE 구문
        ...
        END IF;
*/
-- 사용자에게 점수를 입력받아 SCORE 변수(NUMBER)에 저장한 후 학점은
-- 입력된 점수에 따라 GRADE 변수에 저장
-- 90점 이상은 'A', 80점 이상은 'B', 70점 이상은 'C', 60점 이상은 'D', 60점 미만은 'F'
-- 출력은 '당신의 점수는 95점이고, 학점은 A학점 입니다.'와 같이 출력
DECLARE 
    SCORE NUMBER;
    GRADE CHAR(1);
BEGIN   
...
-- DEPT_CODE에 따라서 알맞는 부서를 출력 (변수명 : DNAME 데이터타입은 VARCHAR2(30))
-- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인 경우 국내영업부
-- D5인 경우 해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부, D8인 경우 기술지원부,
-- D9인 경우 총무부, 나머지는 부서없음
-- 출력은 사번, 이름, 부서코드, 부서가 출력
DECLARE
    EMP EMPLOYEE%ROWTYPE;
    DNAME VARCHAR2(30);
BEGIN 
...
-- TOO_MANY_ROWS : SELECT한 결과가 여러 행인 경우(한 행이 리턴되어야하는데)
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
