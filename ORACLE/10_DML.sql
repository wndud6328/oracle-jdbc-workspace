/*
    ...
    기본값(DEFAULT) 지정되어 있으면 기본값이 들어감
    
    3) INSERT INTO 테이블명 (서브쿼리);
    VALUES 대신 서브쿼리로 조회환 결과값을 통째로 INSERT
...
-- 표현법2)
INSERT INTO EMP_01(EMP_ID, EMP_NAME, DEPT_TITLE, HIRE_DATE) VALUES(400, '임시완', '인사팀', NULL);

INSERT INTO EMP_01(EMP_NAME, EMP_ID, DEPT_TITLE, HIRE_DATE) VALUES('강하늘', 500, '보안팀', SYSDATE);

INSERT INTO EMP_01(EMP_ID, EMP_NAME) VALUES (600, '박성훈');

-- EMP_NAME 컬럼에 NOT NULL 제약조건에 의해 에러 발생 
INSERT INTO EMP_01(EMP_ID, DEPT_TITLE) VALUES(700, '마케팅팀');

INSERT INTO EMP_01(EMP_ID, EMP_NAME, DEPT_TITLE) VALUES(700, '양동근', '마케팅팀');
...
-- EMP_DEPT 테이블에 EMPLOYEE 테이블의 부서 코드가 D1인 직원의 사번, 이름, 부서코드, 입사일을 삽입
...
INSERT ALL
WHEN TO_DATE(20000101) > HIRE_DATE THEN
...
-- DEPT_COPY 테이블에서 DEPT_ID가 'D9'인 부서명을 '전력기획팀'으로 수정
UPDATE DEPT_COPY
SET DEPT_TITLE = '전략기획팀'
WHERE DEPT_ID = 'D9';

-- ALTER은 롤백 안되는데 UPDATE는 가능
ROLLBACK;
...
SET SALARY = '100'
WHERE EMP_NAME='노옹철';
-- EMP_SALARY 선동일 사장의 급여를 7000원으로, 보너스를 0.2로 변경
UPDATE EMP_SALARY
...
UPDATE EMP_SALARY
SET EMP_ID = NULL -- NOT NULL 제약조건에 위배 된다.
WHERE EMP_ID = 200;
...
-- UPDATE 시 서브쿼리 사용 가능
-- 방명수 사원의 급여와 보너스를 유재식 사원가 동일하게 변경

-- 1) 단일 행 서브쿼리를 각각의 컬럼에 적용     
UPDATE EMP_SALARY
SET SALARY = (
        SELECT SALARY
        FROM EMP_SALARY
...
-- DEPARTMENT 테이블에서 DEPT_ID 가 D1인 부서 삭제
...
/*
    ...
    - 별도의 조건을 제시할 수 없고 ROLLVACK 이 불가능하다.
