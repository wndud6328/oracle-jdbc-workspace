...
/*
    1)컬럼 추가/수정/삭제/이름 변경
    1_1) 컬럼 추가(ADD)
        ALTER TALBE 테이블명 ADD 컬럼명 데이터타입 [DEFAULT 기본값];
*/
...
/*
    1_2) 컬럼 수정(MODIFY)
        데이터 타입 변경 : ALTER TALBE 테이블명 MODIFY 컬럼명 변경할 데이터타입;
    기본값 변경 : ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT 변경할 기본값;
                --> DEFAULT NULL : DEFAULT 삭제
*/
-- DEPT_ID 컬럼의 데이터 타입을 CHAR(3)으로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; --오류 발생! 변경할 데이터 타입이 다름
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(10); --오류 발생 //변경하려는 자료형의 크기보다 이미 큰 값이 존재해서 에러
...
-- LOCATION_ID 컬럼의 데이터 타입을 VARCHAR(2),
-- LANME 컬럼의 기본값을 미국으로 변경
...
/*
    ...
    테이블에는 최소 한 개의 컬럼은 존재해야함
*/
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_ID;
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_TITLE;
ALTER TABLE DEPT_COPY DROP COLUMN LOCATION_ID;
ALTER TABLE DEPT_COPY DROP COLUMN CNAME;
ALTER TABLE DEPT_COPY DROP COLUMN LANME; -- 테이블에는 최소 한 개의 컬럼은 존재해야함

ROLLBACK; --롤백하여도 삭제된 컬럼 복구 불가능

ALTER TABLE MEMABER_GRADE DROP COLUMN GRADE_CODE;
...
/*
    ...
    FOREIGN KEY : ADD [CONSTRAINT 제약조건명] PRIMARY KEY(컬럼명) 
                                            REFERENCES 테이블명 [(컬렴명)];
...
*/
-- DPET_COPY 테이블의 DEPT_COPY_DEPT_ID_PK 제약조건 삭제
...
/*
    ...
    3_2) 제약조건명 변경
        ALTER TALBE 테이블명 RENAME CONSTRAINT 기존 계약조건명 TO 변경할 제약조건명;
*/
...
--ALTER는 잘 안쓰이지만 KEY 관리 측면에서 사용 대부분 DROP 하고 CREATE함
...
-- 방법 2. 그냥 부모테이블만 삭제하는데 제약조건까지 같이 삭제하는 방법 (권장안함)
