...
/*
    ...
    - SELECT를 통해헛 조회된 결과를 RESULT SET이라고 한다. 
     (즉, 조회된 행들의 집합)
...
/*
    연결 연산자 : ||
    - 여러 컬럼값들을 마치 하나의 컬럼인 것 처럼 연결하거나,
      컬럼값과 리터럴을 연결할 수 있음
*/
...
SELECT EMP_NAME || '의 월급은' || SALARY || '(급여)원 입니다.' AS "급여정보"
FROM EMPLOYEE;
...
/*
    ...
    !=, ^=, <>      : 같지 않은지 비교
*/
-- EMPLOYEE에서 부서 코드가 'D9'인 사원들만 조회(이때, 모든 컬럼 조회)
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';

-- EMPLOYEE에서 부서 코드가 'D1'인 사원들의 사원명, 급여, 부서코드만 조회
SELECT EMP_NAME, SALARY, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1';

-- 부서코드가 'D1' 이 아닌 사람들의 사번, 사원명, 부서코드 조회
...
-- OR (또는) , AND (그리고)
-- 부서코드가 'D6'이거나 급여가 300만원 이상인 사람들의 
--  사원명, 부서코드 , 급여 조회
...
-- 이름의 가운데 글자가 하 인사람들의 사원명, 전화번호 조회
...
/*
    ...
    8.  OR   (논리연산자)
*/
-- 직급 코드가 J7이거나 J2 인 사원들 중 급여가 200만원 이상인 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE DEPT_CODE >= '95/01/01' AND DEPT_CODE IS NOT NULL;

-- 4. 급여가 200만원 이상 500만원 이하 입사일이 '01/01/01' 이상이고
-- 보너스를 받지 않는 사원들의 사번, 사원명 ,급여, 입사일, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE SALARY BETWEEN 2000000 AND  5000000 AND HIRE_DATE >= '01/01/01' AND BONUS IS NOT NULL ;
...
/*
    ...
    - 실행 순서 : FROM 절 -> WHERE 절 -> SELECT 절 -> ORDER BY절
    ...
    - NULLS FIRST : 정렬하고자하는컬럼값에 NULL이 있을 경우
                        해당 데이터를 맨 앞 배치 ( 생략시 DESC 일때 기본값)
    - NULLS LAST : 정렬하고자하는컬럼값에 NULL이 있을경우
                        해당 데이터를 맨 뒤 배치 ( 생략시 ASC 일때 기본값)
...
FROM EMPLOYEE
--ORDER BY BONUS; -> 보너스 기준 오름차순
ORDER BY BONUS DESC NULLS LAST;  -- 보너스 기준 내림차순(null 이 맨뒤)
