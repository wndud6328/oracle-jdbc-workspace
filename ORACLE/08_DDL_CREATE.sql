...
INSERT INTO MEMBER VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1111-2222',
                                        'aaa@naver.com', '23/06/20');
INSERT INTO MEMBER VALUES(2, 'user02', 'pass02', '홍길녀', '여', null,
                                        NULL, SYSDATE);
INSERT INTO MEMBER VALUES(NULL, NULL, NULL, NULL, NULL, NULL,
                                        NULL, NULL);
SELECT * FROM MEMBER;

/*
    * 제약조건(CONSTRANINTS)
    - 사용자가 원하는 조건에 데이터만 유지하기 위해서 
...
INSERT INTO MEM_NOTNULL VALUES(2, 'user01', 'pass01', '강개똥', null, null, null );
...
-- > UNIQUE 제약조건에 위배 되었으므로 insert 실패!
-- > 오류 구문에 제약조건명으로 알려줌!
-- > 제약조건 부여시 제약조건명을 지정해주지 않으면
-- >      시스템에서 알아서 임의의 제약 조건명을 부여해 버림 
...
/*
    ...
    - 테이블에서 각 행들을 식별하기 위해 사용될 컬럼에 부여하는 제약 조건 (식별자역할)
...
-- > 기본키 중복 값을 담으려고 할 때 (unique 제약조건 위배)
...
/*
    * FOREIGN KEY(외래키) 제약조건
    ...
    -- > 다른 테이블을 참조한다고 표현
    -- > 주로 FOREIGN KEY 제약조건에 의해 테이블 간의 관계가 형성 됨!
    ...
    --> 참조할 걸럼명 생략시 참조할 테이블명에 PRIMARY KEY로 지정된 컬럼으로 매칭
*/
DROP TABLE MEM;
CREATE TABLE MEM(
    MEM_NO NUMBER PRIMARY KEY,
...
--   부모테이블로 부터 무조건 삭제가 안되는 " 삭제제한 " 옵션이 걸려있음
...
INSERT INTO MEMBER VALUES(1, '강길동', 20, '운동', '23/01/01');
...
SELECT MEMBER_NAME, MEMBER_ID, RENT_BOOK_NO, RENT_DATE, RENT_DATE + 7 "TR.RENT_DATE + 7"
    FROM TB_RENT TR, TB_MEMBER TM
    WHERE TM.MEMBER_NO = TR.RENT_BOOK_NO 
    AND RENT_MEM_NO = 2;  
-- 회원번호가 1번인 회원이 대여한 도서
SELECT BK_TITLE, PUB_NAME, RENT_DATE, RENT_DATE+7 "TR.RENT_DATE + 7"
FROM TB_RENT
